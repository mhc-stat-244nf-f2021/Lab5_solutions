...
S <- rep(NA, length(1:11))
I <- rep(NA, length(1:11))
lambdaI <- rep(NA, length(1:11))

S[1]  <- S0
I[1]  <- I0
for (i in 2:11){
  lambdaI[i] <- pc_t*I[i-1]/N
  S[i] <- S[i-1]-lambdaI[i]*S[i-1]
  I[i] <- I[i-1]+lambdaI[i]*S[i-1]
}
```
...
```{r}
t_max <- c(30, 100, 300, 500)

for (tt in t_max){
  S <- rep(NA, length(1:(tt+1)))
  I <- rep(NA, length(1:(tt+1)))
  lambdaI <- rep(NA, length(1:(tt+1)))
  ...
  SI_df <- data.frame(time = rep(0:tt, 2),
                      compartment = rep(c("S", "I"), each=(tt+1)),
                      count = c(S, I))
  print(
    ggplot(data=SI_df, aes(x=time, y=count)) + 
    geom_line(aes(color=compartment))
